...
class Model:
    def __init__(self, train_images, train_labels, num_classes, hp, test_images, test_labels):
        ...
        self.W = np.random.randn(self.input_size, self.num_classes)
        self.b = np.random.randn(1, self.num_classes)
    ...
    def forward_pass(self, inputs):
        ...
        # compute output response to this 'img' input for each neuron
    ...
    def loss(probabilities, gt_label):
        ...
        for j in range(len(probabilities)):
            if j == gt_label:
                loss += -np.log(probabilities[j] + 1e-10)
        return loss
    ...
    def back_propagation(img, probabilities, gt_label):
        ...
        gradW = np.zeros((input_size, num_classes))
        gradB = np.zeros((1, probabilities.shape[0]))

        for j in range(num_classes):
            for i in range(input_size):
                if j == gt_label:
                    gradW[i][j] = np.multiply(img[i], probabilities[j] - 1)
                    gradB[j] = probabilities[j]-1
                else:
                    gradW[i][j] = np.multiply(img[i], probabilities[j])
                    gradB[j] = probabilities[j]
    ...
    def gradient_descent(self, gradW, gradB):
        ...
        self.W = np.subtract(self.W, np.multiply(self.learning_rate, gradW))
        self.b = np.subtract(self.b, np.multiply(self.learning_rate, gradB))

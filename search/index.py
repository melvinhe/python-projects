...
class Indexer:
    ...
    def process_document(self, title: str, id: int, body: str) -> list[str]:
        ...
        """
        # Updating ids_to_titles and titles_to_ids
        self.ids_to_titles[id] = title
        self.titles_to_ids[title] = id
        # Updating ids_to_links
        self.ids_to_links[id] = [self.split_link(link)[1] for link in re.findall(self.tokenization_regex, title + ' ' + body) if self.word_is_link(link)]
        # Tokenizing title and body, removing stopwords, and stemming
        tokens = [self.stem_and_stop(token) for token in re.findall(self.tokenization_regex, title + ' ' + body) if self.stem_and_stop(token) != ""]
        # Updating words_to_doc_frequency (a dict that maps words to dicts where keys are page ids and values are number of times they appear in specific page)
        # Also updating ids_to_max_counts simultaneously!
        ...
        for token in tokens:
            ...
            if id not in self.words_to_doc_frequency[token]:
                self.words_to_doc_frequency[token][id] = 0
            self.words_to_doc_frequency[token][id] += 1
            self.ids_to_max_counts[id] = max(self.ids_to_max_counts[id], self.words_to_doc_frequency[token][id])
    ...
    def parse(self):
        ...
        # You should make no more than two passes through the corpus (that is, have two or fewer separate loops through the pages of the wiki). 
        # Your parse method should call process_document on each document in one of these passes.
        ...
        for wiki_page in wiki_xml_root:
            ...
            self.process_document(page_title, page_id, page_body)
    ...
    def compute_tf(self) -> dict[str, dict[int, float]]:
        ...
        Given a list of all the terms that occur across all of the documents, one can count how many times each term appears in each document. These raw counts are only so useful for comparing documents, however: the term “cat” might appear less often in one document simply because the document is very short. We therefore must normalize the frequency of counts across the documents. We can do this by dividing the count of a term by the count of the most frequently used term in the same document. This is called the term frequency (tf).
        """    
        # TODO: implement this
        term_frequency = {} #dict[str, dict[int, float]]:
        for word in self.words_to_doc_frequency:
            term_frequency[word] = {}
            for page_id in self.words_to_doc_frequency[word]:
                term_frequency[word][page_id] = self.words_to_doc_frequency[word][page_id] / self.ids_to_max_counts[page_id]
    ...
    def compute_idf(self) -> dict[str, float]:
        ...
        for word in self.words_to_doc_frequency:
            inverse_document_frequency[word] = math.log(len(self.words_to_doc_frequency[word]) / len(self.ids_to_titles))
    ...
    def compute_term_relevance(self) -> dict[str, dict[int, float]]:
        ...
        for word in self.words_to_doc_frequency:
            ...
            for id in self.words_to_doc_frequency[word]:
                term_relevance[word][id] = self.compute_tf()[word][id] * self.compute_idf()[word]
    ...
    def compute_page_rank(self) -> dict[int, float]:
        ...
        r_prime = {}
        for id in self.ids_to_titles:
            r_prime[id] = 1 / len(self.ids_to_titles)
        # while distance(r, r') > delta:
        while self.distance(r, r_prime) > self.DISTANCE_THRESHOLD:
            # r <- r'
            r = r_prime
            # for j in pages:
            for j in self.ids_to_titles:
                ...
                r_prime[j] = 0
                for k in self.ids_to_titles:
                    r_prime[j] += self.compute_weights()[k][j] * r[k]
        return r_prime
